...
origin = input("Enter origin station name: ").strip().lower()
dest = input("Enter destination station name: ").strip().lower()

orig_code = station_code_map.get(origin)
dest_code = station_code_map.get(dest)
...
if not orig_code or not dest_code:
    ...
    if "error" in result:
        ...
        print(f"\nCheapest ticket from {origin.title()} to {dest.title()}:")
